...
The full core suite currently has 146 tests across 40 suites. The Milestone 4
...
The local library uses native macOS FSEvents to observe every configured model
root recursively. Bursts are coalesced before a background rescan, so Finder
changes appear without continuous polling; manual Refresh remains available.
